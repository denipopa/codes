// scsBenchC.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
void creationThread() {
	...
	//int P= 100;
	/* create the thread */
	ThreadHandle = CreateThread(NULL,  0,Summation, &N, 0, &ThreadId);
...
void migration() {
		...
		//int P= 100;
		/* create the thread */
		ThreadHandle1 = CreateThread(NULL, 0, Summation, (PVOID)1, 0, &ThreadId1);
		DWORD_PTR p = SetThreadAffinityMask(ThreadHandle1, DWORD_PTR(1) << 1);
		if (p == 0) {
			DWORD err = GetLastError();
			printf("Set affinity failed, err %s \n:", err);
		}
		ThreadHandle2 = CreateThread(NULL, 0, Summation, (PVOID)2, 0, &ThreadId2);
